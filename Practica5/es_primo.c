...
uint8_t es_primo_retardo(uint16_t numero)
{
	uint8_t valor_es_primo = es_primo(numero);
	uint16_t factor_random = rand();
	if(valor_es_primo)
		osDelay(1000 + (2000*factor_random)/0xFFFF);
	return valor_es_primo;
}
